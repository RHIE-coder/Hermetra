...
화면은 뷰포트 높이에 맞고, 위의 줄들이 가져가고 남은 높이를 **편집기가 전부 가진다.**
2026-08-14 까지는 화면이 내용만큼 길어지고 편집기 그리드가 `60vh` 로 고정이었다. 그러면
위의 패널을 접어도 페이지가 짧아질 뿐 파일은 한 줄도 더 못 본다 — 접기를 붙이려면 이
레이아웃이 먼저 바뀌어야 했다.

**넓게 보기**는 파일 트리와 그리드 위 패널(작업대의 브라우저 바)을 접어 편집기에
가로 240px·세로 230px 을 더 준다. 출력 패널은 **접지 않는다**: 그것까지 접으면 넓게
본 채로 누른 `실행` 이 아무 데도 보고하지 못한다.

- `AC-web.code.editor-04` 화면은 뷰포트보다 길어지지 않는다. 트리·편집기·출력은 각자
  자기 안에서 스크롤한다.
- `AC-web.code.editor-05` 넓게 보기는 파일 트리와 그리드 위 패널을 접는다. **출력은 남는다.**
- `AC-web.code.editor-06` 넓게 보기는 `Esc` 로도 나온다. 트리를 감춘 모드를 빠져나오는 데
  그 트리를 감춘 버튼을 찾아야 하면 안 된다.
- `AC-web.code.editor-11` 창을 좁혀도 화면이 **좌우로 스크롤되지 않는다.** 편집기 칸은
  자기 내용보다 작아질 수 있어야 한다 — 편집기가 쥔 열의 폭은 `minmax(0,…)` 이지 맨 `1fr`
  이 아니다. 맨 `1fr` 은 min-content 아래로 안 줄어들고, 이 열의 내용은 **마지막으로 배치된
  폭을 그대로 신고하는 Monaco** 라, 창을 좁히면 열이 옛 폭을 쥔 채 화면 전체가 옆으로
  밀린다(제목과 파일 트리가 왼쪽으로 사라진다). 2026-08-17 피드백.

### `.md` 는 문서로 읽는다

작업대는 스크립트 옆에 가이드를 싣는다 — `GUIDE_ko.md` 와 `GUIDE_en.md`
(`AC-studio.browser-32`, `AC-studio.browser-39`). 그 파일들은 목록의 다른 것과 같은 편집기에서
열리므로, **읽으라고 놓아둔 파일이 `|---|---|` 로** 도착한다. 2026-08-14 에 유저가 그것을
지적했다.

미리보기는 **보기만 한다** — 고치려면 원문으로 돌아간다. 표를 그리려고 GFM
(= GitHub 식 마크다운 확장, 표·취소선 등)을 켠다. 가이드의 절반이 표다.

- `AC-web.code.editor-07` `.md` 를 열면 원문/미리보기 토글이 뜬다. 다른 확장자에는 없다 —
  같은 것을 두 번 보여 주는 선택지는 선택지가 아니다.
- `AC-web.code.editor-08` 미리보기는 제목·목록·**표**·코드·강조를 문서로 그린다.
- `AC-web.code.editor-09` 미리보기의 링크는 **앱을 벗어나** 시스템 브라우저로 간다.
  렌더러가 그 자리에서 이동하면 앱 화면이 통째로 바뀐다.
- `AC-web.code.editor-10` 원문·미리보기 선택과 넓게 보기 상태는 **파일을 바꿔도 유지된다.**
  가이드를 문서로 읽던 사람은 다음 가이드도 문서로 읽는다.
...
- 편집기에 자동 완성·타입 검사가 붙어 있지 않다. Monaco 기본 문법 강조까지다.
  (작업대의 `studio` 슬롯만 예외다 — `docs/spec/studio/browser.md` 참고.)
- 실행은 스크립트 원문을 드라이버로 넘기는 방식이다. 파일 임포트는 지원하지 않는다.
- 마크다운 미리보기는 읽기 전용이다. 문서를 보면서 고칠 수는 없다.
- 넓게 보기와 미리보기 선택은 화면을 떠나면 사라진다. 워크스페이스에 저장하지 않는다.
