...
- `AC-studio.browser-41` 앱이 쓴 그대로인 가이드는 **새 판으로 바뀐다.** 가이드는 사람의
  작업물이 아니라 앱이 싣는 문서라, 한 번 놓고 손 안 대는 규칙만으로는 **틀린 문서가 디스크에
  남는다.** 2026-08-18 에 실제로 그랬다 — 가이드가 `export extract` 를 설명한 채로 놓였고 몇
  시간 뒤 그 기능이 러너에서 사라졌는데, 파일은 그대로 남아 없는 기능을 가르쳤다. 유저가 그
  화면을 찍어 보냈다("아직도 남아있는데...? 없앴다고"). 판정은 시작 파일과 같다 — 앱이 쓴
  바이트와 일치하면 우리 것이라 바꾸고(줄바꿈·끝 공백만 눈감는다), 한 줄이라도 다르면 사람의
  파일이라 안 건드린다. 지금까지 쓴 가이드 원문은 `services/past-guides.ts` 가 든다.
