...
UI 표시명과 내부 식별자가 **둘 다 `브리지 / Bridge`** 다. 라우트(`/bridge/*`)·IPC 채널
(`BRIDGE_*`)·폴더(`modules/bridge`)·도메인 타입(`BridgeEvent`)을 따라 고치면 IPC 계약과
테스트가 통째로 흔들리므로 식별자는 어떤 경우에도 그대로 둔다.

표시명은 한때 **"설정 / Settings"** 였다. 그 아래 든 것이 시나리오·변수·공유 버스·이벤트,
즉 이 제품의 본업 전부인데 "설정"은 "처음에 한 번 만지고 안 보는 곳"으로 읽혀
차별점을 서랍에 넣는 이름이었다. 2026-07-29 에 되돌렸다.
